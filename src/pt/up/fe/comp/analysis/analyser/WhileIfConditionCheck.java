...
public class WhileIfConditionCheck extends PreorderJmmVisitor<Integer, Integer> {
    ...
    public Integer visitIfWhileCondition(JmmNode whileIfStatementNode, Integer ret) {
        JmmNode left_node = whileIfStatementNode.getJmmChild(0);
        ...
        if(isMathExpression || left_node.getKind().equals("Number"))  reports.add(Report.newError(Stage.SEMANTIC, -1, -1, "Invalid type: can't have an int on a while/if statement", null));

        else if(isBooleanExpression) return 1;

        else if(whileIfStatementNode.getJmmChild(1).getKind().equals("DotAccess")){
        ...
        else if(!symbolTable.getVariableType(method_name,left_node.get("name")).getName().equals("boolean")) reports.add(Report.newError(Stage.SEMANTIC, -1, -1, "Invalid type: has to be a boolean", null));
        return 1;
    }
