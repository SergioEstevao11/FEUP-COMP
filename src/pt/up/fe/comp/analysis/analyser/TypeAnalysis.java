...
public class TypeAnalysis extends AJmmVisitor<TypeAnalysis.TypeNScope, Type> {
    ...
    public TypeAnalysis(SymbolTableBuilder symbolTable) {
    ...
    private String getImportName(String importStatement) {
        int delimiterIndex = importStatement.lastIndexOf("/");
    ...
    private Type visitClass(JmmNode node, TypeNScope typeNScope) {
        TypeNScope scope = new TypeNScope(ASTNode.CLASS_SCOPE, NULL, "");
    ...
    private Type visitVar(JmmNode node, TypeNScope typeNScope) {
        ...
        if (variable == null) {
            variable = this.symbolTable.getVariable(AnalysisTable.CLASS_SCOPE, node.get(AnalysisTableBuilder.DELIMITER"VALUE"));
